...
| Authentication Type a| String |  +++The authentication type used to authenticate to MarkLogic. Valid values are: digest, basic, application-level, kerberos.+++ |  | *x*{nbsp}
| SSL Context (Not Yet Supported) a| String |  |  +++null+++ | {nbsp}
| Kerberos External Name (where applicable) a| String |  +++If "kerberos" is used for the authenticationType parameter, a Kerberos external name value can be supplied if needed.+++ |  +++null+++ | {nbsp}
